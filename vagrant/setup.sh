...
if ! grep -q "covidtrack.local.com" /etc/hosts; then
  echo "append to host file"
  echo "" >> /etc/hosts
  echo "127.0.0.1 covidtrack.local.com" >> /etc/hosts
fi
...
cp /home/vagrant/www/covidTrack/vagrant/covidtrack.conf /etc/apache2/sites-available/covidtrack.conf

rm -f /etc/apache2/sites-enabled/covidtrack.conf

ln -s /etc/apache2/sites-available/covidtrack.conf /etc/apache2/sites-enabled/covidtrack.conf


mysql -uroot -ppassword < /home/vagrant/www/covidTrack/vagrant/create.sql
